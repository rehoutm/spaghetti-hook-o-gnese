  ...
  walk(componentNode, (n) => {
    ...
    if (isHookCall(init, "useState") && id?.type === "ArrayPattern") {
      ...
      if (setter?.type === "Identifier") {
        setterNames.add(setter.name as string);
      }
      return true;
    }
...
function scoreReducer(useReducerCall: Node): ReducerShape {
  const args = useReducerCall.arguments as Node[] | undefined;
  const reducerArg = args?.[0];
  const initialArg = args?.[1];

  let slots = 0;
  if (initialArg?.type === "ObjectExpression") {
    const props = initialArg.properties as Node[] | undefined;
  ...
  if (
    ...
    const params = reducerArg.params as Node[] | undefined;
    const stateParamName = params?.[0]?.type === "Identifier"
      ? (params[0].name as string)
      : "state";
    const body = reducerArg.body as Node | undefined;
    if (body) {
      walk(body, (n) => {
        if (n.type !== "SwitchCase") return true;
        const consequent = n.consequent as Node[] | undefined;
